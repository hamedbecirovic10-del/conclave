...
program
    ...
    if (!isLoopback(`http://${opts.host}:${opts.port}`)) {
        console.error(yellow("warning: binding a non-loopback host exposes the room to your network. Conclave is designed to run locally."));
    }
...
identityOptions(program.command("say <text...>").description("broadcast a thought to the project room and exit")).action(async (words, opts) => {
...
identityOptions(program
    .command("ask <text...>")
    .description("ask the room a question and wait for the first answer")
    .option("-t, --timeout <seconds>", "how long to wait", "60")).action(async (words, opts) => {
    const { url, room } = effective(opts);
    try {
        const { result } = await oneShot({ url, room, agent: resolveIdentity(opts), lingerMs: 0 }, (client) => client.ask(words.join(" "), { timeoutMs: Number(opts.timeout) * 1000 }));
        console.log(`${bold(result.from.name)}: ${result.payload.text}`);
    }
    catch (err) {
...
identityOptions(program
    .command("touch <files...>")
    .description("declare files you are about to edit; prints CONFLICT if another agent is on them")).action(async (files, opts) => {
    const { url, room } = effective(opts);
    const { reactions } = await oneShot({ url, room, agent: resolveIdentity(opts), lingerMs: 600 }, (client) => {
...
program
    ...
    if (!data.messages.length)
        console.log(dim(`no messages in ${room}`));
});
function buildClient(room, opts, role) {
...
function attachStream(client) {
    ...
    client.on("reconnecting", ({ attempt, delayMs }) => console.log(yellow(`reconnecting (attempt ${attempt}) in ${delayMs}ms…`)));
    client.on("error", (err) => console.error(red(`error: ${err.message}`)));
}
async function connectAndGreet(client) {
    ...
    rl.on("line", async (line) => {
        ...
        try {
            if (!text) {
                ...
                for (const a of client.roster.values())
                    console.log(formatAgent(a));
            }
            else if (text.startsWith("/status ")) {
                const [status, ...intent] = text.slice(8).split(" ");
                client.status({ status: status, intent: intent.join(" ") || undefined });
            }
            else if (text.startsWith("/activity ")) {
                const [action, targets, ...intent] = text.slice(10).split(" ");
                client.declareActivity({ action: action, targets: (targets ?? "").split(",").filter(Boolean) }, "working", intent.join(" ") || undefined);
            }
                ...
                client
                    .ask(rest, { to })
                    .then((answer) => console.log(green(`answered by ${answer.from.name}: ${answer.payload.text}`)))
                ...
                client.vote(decisionId, vote, rest.join(" ") || undefined);
            }
...
program
    ...
    const room = roomArg ?? ctx.room;
    await requireServer(opts.url);
    const client = buildClient(room, { ...opts, room, provider: "cli", model: "none", role: "observer" }, "observer");
    attachStream(client);
    await connectAndGreet(client);
    const shutdown = async () => {
        await client.disconnect();
...
program
    ...
    .action(async (kind, roomArg, opts) => {
    ...
    const room = roomArg ?? ctx.room;
    let failed = false;
    const { result } = await oneShot({ url: opts.url, room, agent: resolveIdentity(opts), lingerMs: 300 }, (client) => {
        client.on("error", (err) => {
            console.error(red(err.message));
            failed = true;
        });
        return client.publish(parsedKind.data, payload, { to: opts.to, refId: opts.ref });
    });
    if (failed)
        process.exit(1);
...
/* ------------------------------ queries -------------------------------- */
async function apiGet(url, path) {
...
program
    ...
    if (!data.agents.length)
        console.log(dim(`no agents in ${room}`));
});
program
...
program
    ...
    for (const d of data.decisions) {
        const status = d.status === "approved" ? green(d.status) : d.status === "rejected" ? red(d.status) : d.status === "open" ? cyan(d.status) : yellow(d.status);
        console.log(`${bold(d.title)} ${dim(`(${d.id})`)} — ${status}`);
        for (const v of d.votes)
            console.log(`    ${v.agentId}: ${v.vote}${v.comment ? ` — ${v.comment}` : ""}`);
    }
    if (!data.decisions.length)
        console.log(dim("no decisions"));
});
program
    .command("clear [room]")
...
program
    ...
    const room = roomArg ?? ctx.room;
    await requireServer(opts.url);
    const response = await fetch(`${opts.url.replace(/\/+$/, "")}/api/rooms/${encodeURIComponent(room)}/export?format=${opts.format}`);
    if (!response.ok)
        throw new Error(`${response.status} ${await response.text()}`);
