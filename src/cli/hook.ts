...
function readStdin(timeoutMs = 1_500): Promise<string> {
...
export async function runHook(event: string): Promise<void> {
  ...
  try {
    ...
    try {
      payload = raw ? (JSON.parse(raw) as ClaudeHookPayload) : {};
    } catch {
    ...
    if (event === "post-edit") {
      ...
      if (target.startsWith(".conclave") || target === "conclave.json") return;
      const { reactions } = await oneShot(
        { url: ctx.url, room: ctx.room, agent, lingerMs: 600 },
        (client) => {
      ...
      if (alerts.length > 0) {
        const text = alerts
          .map((m) => {
            const p = m.payload as { detail?: string; text?: string; resource?: string };
