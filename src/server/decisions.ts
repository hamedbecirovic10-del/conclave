...
export class DecisionEngine {
  constructor(private store: ConclaveStore) {}
   ...
   */
  castVote(roomId: string, voterAgentId: string, vote: VotePayload): DecisionResolution | { error: string } | null {
  ...
  tryResolve(roomId: string, decisionId: string): DecisionResolution | null {
    ...
    const everyoneVoted =
      electorate.length > 0 && electorate.every((a) => voted.has(a.id));
    if (!everyoneVoted) return null;

    const tally = { approve: 0, reject: 0, request_changes: 0 };
    for (const v of votes) tally[v.vote] += 1;

    let outcome: DecisionOutcome;
    if (tally.reject > 0) outcome = "rejected";
    else if (tally.request_changes > 0) outcome = "changes_requested";
